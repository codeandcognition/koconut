  ...
  render() {
    return (
      <div className="welcome-page">
        ...
        <ul>{strings.use_cases.cases.map((c, i) => <li key={i} id={i}>{c}</li>)}</ul>
        <h4>{strings.user_modes.title}</h4>
        <ol>{strings.user_modes.modes.map((m, i) => <li key={i} id={i}>{m}</li>)}</ol>
        <h4>{strings.language.desc}</h4>
        <p>{strings.language.rationale}</p>
        <h4>{strings.language.example_desc}</h4>
        {strings.language.example_code.map((c, i) => <div key={i} id={i}><code>{c}</code></div>)}
