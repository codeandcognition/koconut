...
// [string]: TestCase
const testCases = new Promise((resolve, reject) => {
  // get all code files
  fs.readdir(test_dir, (err, files) => {
    ...
    async.map(files,
      ...
      (file, callback) => {
        fs.readFile(`${test_dir}/${file}`, (err, data) => {
          if(err) throw err;
...
test('CodeParser.clean', () => {
  testCases.then((tests) => {
    Object.keys(tests).forEach((test) => {
      console.log(`Testing ${test}`);
      expect(CodeParser.clean(tests[test].input)).toBe(tests[test].output);
    });
  });
});